class NLPTermsExtractor {
    ...
    bindEvents() {
        ...
        this.uploadArea.addEventListener('click', (e) => {
            ...
            if (e.target.id === 'zipNameInput' || e.target.id === 'downloadAllBtn') return;
    ...
    extractTerms(text) {
        ...
        if (!terms) {
            return { terms: "No terms found in the IMPORTANT TERMS section.", count: 0 };
        }

        const cleanedTerms = [...new Set(terms.map(term => term.replace(/^\*\s*/, '').replace(/:$/, '').trim()))];
    ...
    createResultCard(result, index) {
        ...
        card.className = 'bg-white rounded-2xl shadow-lg p-6 card-hover animate-slide-up';
        ...
        return `
            <div class="flex items-start space-x-3 mb-4">
                <div class="flex-1 min-w-0">
                    <h3 class="font-semibold text-gray-800 text-sm font-roboto" style="font-family: 'Roboto', sans-serif;">
            ...
            <div class="bg-gray-50 rounded-xl p-3 mb-4 h-32 overflow-y-auto">
                <pre class="text-xs text-gray-700 font-mono whitespace-pre-wrap">${result.terms}</pre>
            </div>
            
            <div class="flex items-center justify-between">
                <span class="bg-gradient-to-r from-indigo-500 to-purple-600 text-white text-xs font-semibold px-4 py-2 rounded-full shadow-md" style="font-family: 'Roboto', sans-serif;">
    ...
    extractTerms(text) {
        ...
        let cleanedTerms = terms.map(term => term.replace(/^\*\s*/, '').replace(/:$/, '').trim());
    ...
    async downloadAllTerms() {
        ...
        try {
            ...
            cards.forEach((card, index) => {
                ...
                const cleanFilename = `${title.replace(/_/g, ' ').replace(/\s+/g, ' ').trim()}.txt`;
